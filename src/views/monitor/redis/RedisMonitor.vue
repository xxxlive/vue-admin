<template>
  <div style="width: 100%;margin-top: 1rem">
    <a-row :gutter="8">
      <a-col :span="12">
        <apexchart ref="memoryInfo" :options="memory.chartOptions" :series="memory.series" type="area" height="350"/>
      </a-col>
      <a-col :span="12">
        <apexchart ref="keySize" :options="key.chartOptions" :series="key.series" type="area" height="350"/>
      </a-col>
    </a-row>
    <a-row :gutter="8">
...
import redis from '@/api/monitor/redis'
...
export default {
  ...
  mounted() {
    ...
    this.timer = setInterval(() => {
      if (this.$route.path.indexOf('/monitor/redis') !== -1) {
        axios.all([
          redis.getKeySize(),
          redis.getMemoryInfo()
        ]).then((r) => {
          const currentMemory = r[1].data.data.used_memory / 1000
          const currentSize = r[0].data.data.dbSize
    ...
    redis.getRedisInfo().then((r) => {
      const tem = []
      const infos = ['redis_version', 'os', 'arch_bits', 'process_id']
      for (let i = 0; i < r.data.data.length; i++) {
        const t = r.data.data[i]
        if (infos.indexOf(t.key) !== -1) {
          tem.push(r.data.data[i])
        }
      }
      this.redisInfo = tem
